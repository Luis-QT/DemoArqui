<div class="panel panel-default">
    <div class="panel-heading">LISTA DE AUTORES
    <a data-id="" class="btn btn-success  pull-right" href='{{url("/admin/authors/export")}}' target="_blank" onclick="window.open(this.href, this.target, 'width=900,height=600'); return false;"><i class="fa fa-file-pdf" ></i>Exportar</a>
    </div>
              ...
              <tbody>
                  @foreach($autores as $i => $autor)
                    <tr>
                      ...
                      <td>{{ $autor->name }}</td>
                      <td>@foreach($autor->categories as $i => $categoria)
                            @if($i>0)
                              {{' , '}}
                            @endif
                            {{$categoria->name}}
                      ...
                      <td>
                        <button type="button" data-id="{{$autor->id}}" class="btn btn-success editar"><i class="fa fa-pencil"></i></button>
                      </td>
                      <td>
                        <a type="button" data-id="{{$autor->id}}"  data-name="{{ $autor->name }}" class="btn btn-danger eliminar">
